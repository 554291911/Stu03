 ...
 */
@Controller
@RequestMapping("/cxjllr")
public class CxjllrController {
	...
	public String index() {
		return "classes/cn/edu/xiyou/cxjllr/index";
	}
	...
	public void load(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		List<Credit> credits = creditDao.findByLrryUsername(user.getUsername());
	...
	public void addCXJL(String xh,String text,Integer fs, Writer writer, HttpServletRequest request) throws IOException {
		User user = (User) request.getSession().getAttribute("user");
		String s = "ok";
	...
	public void getstuinfo(String xh, Writer writer,HttpServletRequest request) throws IOException {
		...
		if (xh != null && !xh.equals("")) {
			Student student = studentDao.findBySnumber(xh);
			if (student != null) {
				if(user.getCollege().equals(student.getCollege())){
					s = JSON.toJSONString(student);
				}else{
					s=JSON.toJSONString("limit");
				}
			}
		}
		writer.append(s);
	...
	public String index(HttpServletResponse response,Map<String, Object> map,String id){
		...
		return "classes/cn/edu/xiyou/cxjllr/word";
	}
}
